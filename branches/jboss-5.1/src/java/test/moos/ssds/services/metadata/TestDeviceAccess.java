...
public class TestDeviceAccess extends TestAccessCase {
	...
	String deviceThreeStringRep = "Device|" + "name=Like Name One|"
			...
			+ "mfgModel=LikeNameOneMFGModel|"
			+ "mfgSerialNumber=LikeNameOneMFGSN|";
	String deviceFourStringRep = "Device|" + "name=Like Name Two|"
			+ "description=Like Name Two Description|" + "mfgName=LikeNameMFG|"
			+ "mfgModel=LikeNameTwoMFGModel|"
			+ "mfgSerialNumber=LikeNameTwoMFGSN|";
	...
	protected void setUp() {
		...
		try {
			...
			deviceTypeAccess = (DeviceTypeAccess) context
					.lookup("moos/ssds/services/metadata/DeviceTypeAccess");
		} catch (NamingException e) {
			logger.error("CreateException caught while creating Access interfaces: "
					+ e.getMessage());
		}
	...
	public void testIDGeneration() {
		...
		assertTrue("The first and second IDs should not be the same",
				deviceID.longValue() != deviceIDSecondInsert.longValue());
	...
	public void testRelationships() {
		...
		try {
			...
			logger.error("MetadataAccessException caught inserting devices in relationship test: "
					+ e.getMessage());
			assertTrue(
		...
		try {
			...
			logger.error("MetadataAccessException caught trying to read back devices: "
					+ e.getMessage());
			assertTrue(
		...
		try {
			...
			logger.error("MetadataAccessException caught trying to update device two: "
					+ e.getMessage());
			assertTrue(
					"MetadataAccessException caught trying to read back devices: "
							+ e.getMessage(), false);
		...
		try {
			...
			logger.error("MetadataAccessException caught trying to delete the devices:"
					+ e.getMessage());
			assertTrue(
		...
		try {
			...
			logger.error("MetadataAccessException caught trying to find"
					+ " the people after deleting the devices: "
					+ e.getMessage());
			assertTrue(
					"MetadataAccessException caught trying to find"
							+ " the people after deleting the devices: "
							+ e.getMessage(), false);
		}
		...
		try {
			...
			logger.error("MetadataAccessException caught trying to delete the people: "
					+ e.getMessage());
			assertTrue(
		...
		try {
			...
			logger.error("MetadataAccessException caught trying to read back device object graph: "
					+ e.getMessage());
			assertTrue(
		...
		try {
			...
			logger.error("MetadataAccessException caught trying to find the updated device:"
					+ e.getMessage());
			assertTrue(
		...
		// Check find bys
		logger.debug("OK, devices deleted, let's see about finding the deviceTypes");
		...
		try {
			...
			logger.error("MetadataAccessException caught trying to delete the deviceType: "
					+ e.getMessage());
			assertTrue(
	...
	public void testFindBys() {
		...
		try {
			persistedDeviceOne = (Device) deviceAccess.findById(
					deviceOneId.longValue(), false);
		} catch (MetadataAccessException e) {
			assertTrue(
					"MetadataAccessException caught trying to findById(long):"
							+ e.getMessage(), false);
		...
		persistedDeviceOne = null;
		try {
			persistedDeviceOne = (Device) deviceAccess.findByUuid(
					deviceOne.getUuid(), false);
		} catch (MetadataAccessException e) {
			assertTrue(
		...
		assertEquals("Id of the equivalent persistent object"
				+ " should match that of insert", deviceOneId,
				equivalentDeviceOne.getId());
		...
		assertTrue("findAll should have deviceOne",
				allDevices.contains(deviceOne));
		assertTrue("findAll should have deviceTwo",
				allDevices.contains(deviceTwo));
		assertTrue("findAll should have deviceThree",
				allDevices.contains(deviceThree));
		assertTrue("findAll should have deviceFour",
				allDevices.contains(deviceFour));
		assertTrue("findAll should have deviceFive",
				allDevices.contains(deviceFive));
		assertTrue("findAll should have deviceSix",
				allDevices.contains(deviceSix));
		...
		assertTrue("findAll should NOT have deviceOne",
				!likeNameDevices.contains(deviceOne));
		assertTrue("findAll should NOT have deviceTwo",
				!likeNameDevices.contains(deviceTwo));
		assertTrue("findAll should have deviceThree",
				likeNameDevices.contains(deviceThree));
		assertTrue("findAll should have deviceFour",
				likeNameDevices.contains(deviceFour));
		assertTrue("findAll should have deviceFive",
				likeNameDevices.contains(deviceFive));
		assertTrue("findAll should have deviceSix",
				likeNameDevices.contains(deviceSix));
		...
		assertTrue("There should be two in the return",
				byNameAndMFGStuff.size() == 2);
		assertTrue("Device three should be there",
				byNameAndMFGStuff.contains(deviceThree));
		assertTrue("Device four should be there",
				byNameAndMFGStuff.contains(deviceFour));
		...
		assertTrue("There should be one in the return",
				byNameAndMFGStuff.size() == 1);
		assertTrue("Device three should be there",
				byNameAndMFGStuff.contains(deviceThree));
		...
		try {
			byNameAndMFGStuff = deviceAccess.findByNameAndMfgInfo(null, false,
					null, false, null, false, "LikeNameMFGSN", false, "id",
					null, false);
		} catch (MetadataAccessException e1) {
			assertTrue(
					"MetadataAccessException caught trying to findByNameAndMfgInfo():"
		...
		assertTrue("There should be two in the return",
				byNameAndMFGStuff.size() == 2);
		assertTrue("Device five should be there",
				byNameAndMFGStuff.contains(deviceFive));
		assertTrue("Device six should be there",
				byNameAndMFGStuff.contains(deviceSix));
		...
		assertTrue("allIds should have six (or more) entries.",
				allIds.size() >= 6);
		assertTrue("allNames should have six (or more) entries",
				allNames.size() >= 6);
		// Now make sure the ids and names are all in the returned collection
		assertTrue("Device ID one should be there: ",
				allIds.contains(deviceOneId));
		assertTrue("Device ID two should be there: ",
				allIds.contains(deviceTwoId));
		assertTrue("Device ID three should be there: ",
				allIds.contains(deviceThreeId));
		assertTrue("Device ID four should be there: ",
				allIds.contains(deviceFourId));
		assertTrue("Device ID five should be there: ",
				allIds.contains(deviceFiveId));
		assertTrue("Device ID six should be there: ",
				allIds.contains(deviceSixId));
		assertTrue("Device name one should be there: ",
				allNames.contains("DeviceOne"));
		assertTrue("Device name two should be there: ",
				allNames.contains("DeviceTwo"));
		assertTrue("Device name two should be there: ",
				allNames.contains("Like Name One"));
		assertTrue("Device name two should be there: ",
				allNames.contains("Like Name Two"));
		assertTrue("Device name two should be there: ",
				allNames.contains("Like Name Three"));
		assertTrue("Device name two should be there: ",
				allNames.contains("Like Name Four"));
		...
		assertTrue("DeviceOne should be there",
				devicesByPerson.contains(deviceOne));
		assertTrue("DeviceTwo should be there",
				devicesByPerson.contains(deviceTwo));
		assertTrue("DeviceThree should NOT  be there",
				!devicesByPerson.contains(deviceThree));
		assertTrue("DeviceFour should be there",
				devicesByPerson.contains(deviceFour));
		assertTrue("DeviceFive should be there",
				devicesByPerson.contains(deviceFive));
		assertTrue("DeviceSix should be there",
				devicesByPerson.contains(deviceSix));
		...
		assertTrue("DeviceOne should be there",
				devicesByDeviceType.contains(deviceOne));
		assertTrue("DeviceTwo should be there",
				devicesByDeviceType.contains(deviceTwo));
		assertTrue("DeviceThree should NOT be there",
				!devicesByDeviceType.contains(deviceThree));
		assertTrue("DeviceFour should be there",
				devicesByDeviceType.contains(deviceFour));
		assertTrue("DeviceFive should be there",
				devicesByDeviceType.contains(deviceFive));
		assertTrue("DeviceSix should be there",
				devicesByDeviceType.contains(deviceSix));
		// Now clean up
		this.cleanObjectsFromDB();
	}
	...
	private void setupObjects() {
		try {
			...
			logger.error("ClassCastException caught trying to create two Person objects: "
					+ cce.getMessage());
		}
	}
