...
public class NetcdfConverter {
	...
	/**
	 * This is a <code>ITimeIndexedDataAccess</code> object that simplifies the
	 * construction of the NetCDF file. I the <code>DataContainer</code> is time
	 * based, the NetCDF will use a time dimension, otherwise, it will use an
	 * index dimension.
	 */
	...
	/**
	 * DataProducerAccess services
	 */
	private DataProducerAccessLocal dpal = null;
	...
	public NetcdfConverter(DataContainer dataContainer, File netcdfFile)
		...
		logger.debug("NetCDFCreate instantiated with DataContainer: "
				+ dataContainer.toStringRepresentation("|")
				+ " and NetCDF file: " + netcdfFile.toString());

		// Connect up to EJB services
		try {
			Context context = new InitialContext();
			this.dpal = (DataProducerAccessLocal) context
					.lookup("moos/ssds/services/metadata/DataProducerAccessLocal");
			if (this.dpal == null) {
				this.dpa = (DataProducerAccess) context
						.lookup("moos/ssds/services/metadata/DataProducerAccess");
			}
		} catch (NamingException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			logger.error("NamingException caught trying to connect to SSDS services: "
					+ e.getMessage());
		}
	}
	...
	public void create() throws IOException {
		...
		try {
			logger.debug("Going to build TimeIndexedDataAccess for the DataContainer");
			this.timeIndexedDataAccess = TimeIndexedDataAccessFactory
					.getTimeIndexedDataAccess(this.dataContainer, null, null,
							null);
		} catch (DataException e) {
			logger.debug("Could not create a time indexed data access for the NetCDF creator");
			this.timeIndexedDataAccess = null;
		} catch (Exception e) {
			logger.error("Exception caught trying to build time indexed data access: "
					+ e.getMessage());
		}
		...
		if (timeIndexedDataAccess != null) {
			...
			while (rvIter.hasNext()) {
				...
				try {
					...
					logger.error("IOException caught trying to write data to variable "
		...
		if ((producer != null)
			...
			if (instrument != null) {
				ncfile.addGlobalAttribute("instrumentId", instrument.getId()
						+ "");
				ncfile.addGlobalAttribute("instrumentUUID",
						instrument.getUuid() + "");
				ncfile.addGlobalAttribute("instrumentName",
						instrument.getName() + "");
				if (instrument.getDeviceType() != null) {
					ncfile.addGlobalAttribute("instrumentType", instrument
							.getDeviceType().getName() + "");
				}
				ncfile.addGlobalAttribute("instrumentMfg",
						instrument.getMfgName() + "");
				ncfile.addGlobalAttribute("instrumentSerialNumber",
						instrument.getMfgSerialNumber() + "");
				ncfile.addGlobalAttribute("instrumentModel",
						instrument.getMfgModel() + "");
			}
		}
	}
	...
	private void write(RecordVariable recordVariable, Object[] dataValues)
		...
		if (number != null) {
			...
			if (number instanceof Byte) {
				...
				netcdfWriteable.write(recordVariable.getName(),
						ArrayAbstract.factory(byteArray));
				...
				netcdfWriteable.write(recordVariable.getName(),
						ArrayAbstract.factory(shortArray));
				...
				netcdfWriteable.write(recordVariable.getName(),
						ArrayAbstract.factory(intArray));
				...
				// Write to NetCDF File
				netcdfWriteable.write(recordVariable.getName(),
						ArrayAbstract.factory(longArray));
				...
				// Write to NetCDF File
				netcdfWriteable.write(recordVariable.getName(),
						ArrayAbstract.factory(floatArray));
				...
				// Write to NetCDF file
				netcdfWriteable.write(recordVariable.getName(),
						ArrayAbstract.factory(doubleArray));
