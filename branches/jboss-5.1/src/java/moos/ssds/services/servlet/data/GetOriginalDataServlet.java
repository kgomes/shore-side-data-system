...
public class GetOriginalDataServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (startTimestampSecondsParameter != null) {
			...
			try {
				...
				logger.debug("Could not convert startTimestampSecondsParameter of "
		...
		if (startTimestampNanosecondsParameter != null) {
			try {
				...
				logger.debug("Could not convert startTimestampNanosecondsParameter of "
		...
		if (endTimestampSecondsParameter != null) {
			...
			try {
				...
				logger.debug("Could not convert endTimestampSecondsParameter of "
		...
		if (endTimestampNanosecondsParameter != null) {
			try {
				...
				logger.debug("Could not convert endTimestampNanosecondsParameter of "
		...
		try {
			...
			dataMap = sqlDSLocal.getSortedRawData(deviceID, startParentID,
					...
					startDataDescriptionID, endDataDescriptionID,
					startDataDescriptionVersion, endDataDescriptionVersion,
					...
					lastNumberOfPackets, startLatitude, endLatitude,
					startLongitude, endLongitude, startDepth, endDepth,
					orderBy, returnAsSSDSDevicePackets);
	 ...
	 *         <code>String</code> (if possible). If no conversion was possible,
	 *         null is returned.
	 */
		...
		if (dateToReturn == null) {
			...
			try {
				calendar.set(Calendar.YEAR,
						Integer.parseInt(dateParameter.substring(0, 4)));
				calendar.set(Calendar.MONTH,
						Integer.parseInt(dateParameter.substring(4, 6)) - 1);
				calendar.set(Calendar.DAY_OF_MONTH,
						Integer.parseInt(dateParameter.substring(6, 8)));
				calendar.set(Calendar.HOUR_OF_DAY,
						Integer.parseInt(dateParameter.substring(9, 11)));
				calendar.set(Calendar.MINUTE,
						Integer.parseInt(dateParameter.substring(11, 13)));
				calendar.set(Calendar.SECOND,
						Integer.parseInt(dateParameter.substring(13, 15)));
	...
	private void respondAscii(TreeMap dataMap, PrintWriter out,
		...
		// The data packets are mapped by key which may be a timestamp or
		// sequence number.
		...
		while (i.hasNext()) {
			...
			// Skip records using a remainder test on the stride specified in
			// the request
			// Make sure to increment the iterator!
			counter++;
			Long key = (Long) i.next();
			// out.println("counter = " + counter + " strideCount = " +
			// strideCount + " mod div = " + (counter % strideCount));
			...
			// Now loop over the values for the key - these values may consist
			// of multiple RecordTypes
			...
			if (values != null) {
				...
				while (iterator.hasNext()) {
					...
					if ((prependWith != null)
							&& (prependWith
									.equalsIgnoreCase(PREPEND_WITH_OASIS))) {
	...
	private void respondBinary(TreeMap dataMap, OutputStream out,
			HttpServletResponse response, String recordDelimiter,
			int strideCount) {
			...
			if (values != null) {
				...
				while (iterator.hasNext()) {
					...
					try {
						...
						logger.error("IOException caught while writing back binary requested data: "
								+ e.getMessage());
					}
				}
			}
	...
	public void outputUsage(String baseURLPath, String htmlErrorMessage,
		...
		usageMessageBuffer.append("<tr><th>Parameter Name</th>"
				+ "<th>Opt/Required</th>"
				+ "<th>Query or<br>Return Format</th>"
		...
		usageMessageBuffer
				.append("<tr><td>outputAs</td>"
						...
						+ " be in ASCII</td></tr>");
		usageMessageBuffer
				.append("<tr><td>displayPacketHeaderInfo</td>"
						+ "<td>optional</td>"
						+ "<td>return<br>format</td>"
						+ "<td>true|false</td>"
