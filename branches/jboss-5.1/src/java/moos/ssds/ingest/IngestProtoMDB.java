 ...
 * organized by certain &quot;keys&quot; (if enabled) as well as in a relational
 * database. It then passes these message on to more topics for further
 * processing.
 * </p>
 * <hr>
 * 
...
public class IngestProtoMDB implements MessageListener {
	 ...
	 * A log4j logger
	 */
	static Logger logger = Logger.getLogger(IngestProtoMDB.class);
	...
	public void onMessage(javax.jms.Message msg) {
		...
		if (msg instanceof BytesMessage) {
			...
			// Now this message will be in the protocol buffer format so I will
			// need to de-serialize it. First grab the full byte array
			byte[] protoMessage = null;
			try {
				protoMessage = new byte[(int) bytesMessage.getBodyLength()];
				bytesMessage.readBytes(protoMessage);
			} catch (JMSException e3) {
				logger.error("JMSException caught trying to read bytes from message: "
						+ e3.getMessage());
			}

			// If the bytes were extracted, parse them into an protocol buffer
			// message
			...
			if (protoMessage != null) {
				try {
					...
					logger.error("InvalidProtocolBufferException caught "
							+ "trying to parse protocol buffer: "
							+ e1.getMessage());
				}
			}

			// Read in the keys for the metadata packet
			if (ssdsGeoLocatedDevicePacketProto != null) {
				// Convert it from the protocol buffers form to the SSDS format
				// of byte array
				byte[] ssdsFormatByteArray = PacketUtility
						.convertProtocolBuffersByteArrayToSSDSByteArray(ssdsGeoLocatedDevicePacketProto
								.toByteArray());
				...
				// Grab the keys from the protocol buffer message
				deviceID = ssdsGeoLocatedDevicePacketProto.getSourceID();
				parentID = ssdsGeoLocatedDevicePacketProto.getParentID();
				packetType = ssdsGeoLocatedDevicePacketProto.getPacketType();
				packetSubType = ssdsGeoLocatedDevicePacketProto
						.getPacketSubType();
				dataDescriptionID = ssdsGeoLocatedDevicePacketProto
						.getDataDescriptionVersion();

				// Check to see if the message should be serialized to disk
				if (fileSerializationEnabled)
					persistBytesMessageToFile(deviceID, parentID,
							packetSubType, dataDescriptionID,
							ssdsFormatByteArray);

				// Now "Serialize" the packet to the database
				persistBytesMessageToDatabase(deviceID, ssdsFormatByteArray);
				...
				if (packetType == 1) {
					...
					try {
						BytesMessage ruminateMessage = session
								.createBytesMessage();
						bytesMessage.writeBytes(ssdsFormatByteArray);
						messageProducer.send(ruminateMessage);
					} catch (JMSException e2) {
						logger.error("JMSException caught while trying "
								+ "to publish the bytes message"
								+ e2.getMessage());
					}
				}
			}
	...
	private void persistBytesMessageToFile(long deviceID, long parentID,
		...
		try {
			po.writeBytes(ssdsFormatByteArray);
