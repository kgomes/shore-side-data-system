...
<body id="body">
	...
	<%
		...
		if ((personAccessLocal != null) && (deviceAccessLocal != null)
				&& (deviceTypeAccessLocal != null)) {

			// Grab all the persons from the SSDS database
			Collection results = null;
			try {
				results = personAccessLocal
			...
			try {
				results = deviceTypeAccessLocal.findAll("name", "asc",
						false);
			} catch (Exception dae) {
			}
			...
			if (results != null) {
				...
				while (i.hasNext()) {
					...
					deviceTypeInputBuffer.append("<option value=\""
							+ deviceType.getId() + "\">"
							+ deviceType.getName() + " ("
							+ deviceType.getDescription() + ")</option>");
				}
			}
		} else {
	...
	<center>
		<form name="newDeviceForm" method="post" action="addDevice.jsp">
			<table width="75%" border="3" cellspacing="1" cellpadding="1">
				...
				<tr>
					<td>&nbsp;<strong>Device Description</strong>
					</td>
					<td><input type="text" name="description" />
					</td>
				</tr>
				<tr>
					<td>&nbsp;<strong>Manufacturer's Name</strong>
					</td>
					<td><input type="text" name="mfgName" />
					</td>
				</tr>
				<tr>
					<td>&nbsp;<strong>Manufacturer's Model</strong>
					</td>
					<td><input type="text" name="mfgModel" />
					</td>
				</tr>
				<tr>
					<td>&nbsp;<strong>Manufacturer's Serial Number</strong>
					</td>
					<td><input type="text" name="mfgSerialNumber" />
					</td>
				</tr>
				<tr>
