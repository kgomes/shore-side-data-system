...
<script type="text/javascript"
	src="/ssds/theme/com/sun/rave/web/ui/defaulttheme/javascript/formElements.js"></script>
<link rel="stylesheet" type="text/css"
	href="/ssds/theme/com/sun/rave/web/ui/defaulttheme/css/css_master.css" />
<link rel="stylesheet" type="text/css"
	href="resources/ssds_stylesheet.css"></link>
</head>
<body id="body" style="-rave-layout: grid">
	...
	<%
		...
		Logger logger = Logger
				.getLogger("moos.ssds.web.siamRawDataStep2.jsp");

		// Log the input parameters
		logger.debug("Input parameters (Stuff in parens is extracted from query params):");

		// First grab and log the query variables
		// First grab the device ID that was entered
		String deviceIDEntered = request.getParameter("deviceIDEntered");
		String deviceID = request.getParameter("deviceID");
		if ((deviceIDEntered != null)
				&& (deviceIDEntered.compareToIgnoreCase("") != 0)) {
			deviceID = deviceIDEntered;
		}
		logger.debug(" --> deviceID=" + deviceID + " (deviceIDEntered="
				+ request.getParameter("deviceIDEntered") + ", deviceID="
		...
		if (orderBy.compareTo("sequenceNumber") == 0) {
			orderByTimeBoolean = false;
		}
		logger.debug(" --> orderBy=" + orderBy + ", orderByTimeBoolean="
				+ orderByTimeBoolean + " (tsORSeq="
				+ request.getParameter("tsORseq") + ")");
		...
		if ((displayTimestamp != null)
				&& (displayTimestamp.compareTo("true") == 0)) {
			displayTimestampBoolean = true;
		}
		logger.debug(" -->displayTimestamp=" + displayTimestamp
				+ ", displayTimeStampBoolean=" + displayTimestampBoolean
				+ " (displayTimestamp="
				+ request.getParameter("displayTimestamp") + ")");
		...
		logger.debug(" --> showDate=" + showDate + ", showDateBoolean="
				+ showDateBoolean + " (showDate="
				+ request.getParameter("showDate") + ")");

		// Grab the user input for whether or not they want the sequence number
		String displaySequenceNumber = request
				.getParameter("displaySequenceNumber");
		boolean displaySequenceNumberBoolean = false;
		if ((displaySequenceNumber != null)
				&& (displaySequenceNumber.compareTo("true") == 0)) {
			displaySequenceNumberBoolean = true;
		}
		logger.debug(" --> displaySequenceNumber=" + displaySequenceNumber
				+ ", displaySequenceNumberBoolean="
				+ displaySequenceNumberBoolean + " (displaySequenceNumber="
				+ request.getParameter("displaySequenceNumber") + ")");
		...
		logger.debug(" --> timezone=" + timezone + "(timezone="
				+ request.getParameter("timezone") + ")");

		// Grab teh format for date and time
		String dateTimeConversionFormat = request
				.getParameter("dateTimeConversionFormat");
		logger.debug(" --> dateTimeConversionFormat="
				+ dateTimeConversionFormat + "(dateTimeConversionFormat="
				+ request.getParameter("dateTimeConversionFormat") + ")");

		// Now grab the user input for whether or not they want the parent ID displayed
		String displayParentIDString = request
				.getParameter("displayParentID");
		boolean displayParentID = false;
		if ((displayParentIDString != null)
				&& (displayParentIDString.compareTo("true") == 0)) {
			displayParentID = true;
		}
		logger.debug(" --> displayParentID=" + displayParentID
				+ "(displayParentID="
				+ request.getParameter("displayParentID") + ")");

		// Now grab the user input for whether or not they want the RecordType displayed
		String displayRecordTypeString = request
				.getParameter("displayRecordType");
		boolean displayRecordType = false;
		if ((displayRecordTypeString != null)
				&& (displayRecordTypeString.compareTo("true") == 0)) {
			displayRecordType = true;
		}
		logger.debug(" --> displayRecordType=" + displayRecordTypeString
				+ request.getParameter("displayRecordType"));

		// Now grab the user input for whether or not they want the PacketType displayed
		String displayPacketTypeString = request
				.getParameter("displayPacketType");
		boolean displayPacketType = false;
		if ((displayPacketTypeString != null)
				&& (displayPacketTypeString.compareTo("true") == 0)) {
			displayPacketType = true;
		}
		logger.debug(" --> displayPacketType=" + displayPacketType
				+ "(displayPacketType="
				+ request.getParameter("displayPacketType") + ")");

		// Now grab the user input for whether or not they want primary data buffer displayed
		String displayDataBufferString = request
				.getParameter("displayDataBuffer");
		boolean displayDataBuffer = true;
		if ((displayDataBufferString != null)
				&& (displayDataBufferString.compareTo("false") == 0)) {
			displayDataBuffer = false;
		}
		logger.debug(" --> displayDataBuffer=" + displayDataBuffer
				+ "(displayDataBuffer="
				+ request.getParameter("displayDataBuffer") + ")");

		// Now grab the user input for whether or not they want the data buffer size displayed
		String showDataBufferSize = request
				.getParameter("showDataBufferSize");
		boolean showDataBufferSizeBoolean = false;
		if ((showDataBufferSize != null)
				&& (showDataBufferSize.compareTo("true") == 0)) {
			showDataBufferSizeBoolean = true;
		}
		logger.debug(" --> showDataBufferSize=" + showDataBufferSizeBoolean
				+ " (showDataBufferSize="
				+ request.getParameter("showDataBufferSize") + ")");
		...
		if ((convertTo != null)
				&& (convertTo.compareTo("hex") == 0)
				|| ((convertToHex != null) && (convertToHex
						.compareTo("yes") == 0))) {
			convertToHexBoolean = true;
		} else if ((convertTo != null)
				&& (convertTo.compareTo("imageTag") == 0)) {
			convertToImageTag = true;
		}
		logger.debug(" --> convertToHex=" + convertToHexBoolean
				+ " (convertToHex=" + request.getParameter("convertToHex")
		...
		String displaySecondDataBufferString = request
				.getParameter("displaySecondaryDataBuffer");
		boolean displaySecondDataBuffer = false;
		if ((displaySecondDataBufferString != null)
				&& (displaySecondDataBufferString.compareTo("true") == 0)) {
			displaySecondDataBuffer = true;
		}
		logger.debug(" --> displaySecondaryDataBuffer="
				+ displaySecondDataBuffer + " (displaySecondaryDataBuffer="
				+ request.getParameter("displaySecondaryDataBuffer") + ")");

		// Now grab the user input for whether or not they want the second data buffer converted to hex
		String convertSecondToHexString = request
				.getParameter("convertSecondaryToHex");
		boolean convertSecondToHexBoolean = false;
		if ((convertSecondToHexString != null)
				&& (convertSecondToHexString.compareTo("yes") == 0)) {
			convertSecondToHexBoolean = true;
		}
		logger.debug(" --> convertSecondaryToHex="
				+ convertSecondToHexBoolean + "( convertSecondaryToHex="
				+ request.getParameter("convertSecondaryToHex") + ")");

		// Now grab the user input for whether or not they want the size of the second data buffer displayed
		String displaySecondDataBufferSizeString = request
				.getParameter("displaySizeOfSecondaryDataBuffer");
		boolean displaySecondDataBufferSize = false;
		if ((displaySecondDataBufferSizeString != null)
				&& (displaySecondDataBufferSizeString.compareTo("true") == 0)) {
			displaySecondDataBufferSize = true;
		}
		logger.debug(" --> displaySecondDataBufferSize="
				+ displaySecondDataBufferSize
				+ " (displaySizeOfSecondaryDataBuffer="
				+ request.getParameter("displaySizeOfSecondaryDataBuffer")
				+ ")");
		...
		logger.debug(" --> recordType=" + recordType + " (recordType="
		...
		logger.debug(" --> timeQueryBy=" + timeQueryBy + " (timeQueryType="
				+ request.getParameter("timeQueryType") + ")");

		// Grab the number of packets back
		String numberOfPacketsString = request
				.getParameter("lastNumPackets");
		...
		logger.debug(" --> lastNumPackets=" + numberOfPackets
				+ " (lastNumPackets="
				+ request.getParameter("lastNumPackets") + ")");

		// Grab the start time
		String startTime = request.getParameter("startTime");
		logger.debug(" --> (startTime=" + request.getParameter("startTime")
				+ ")");

		// Grab the end time
		String endTime = request.getParameter("endTime");
		logger.debug(" --> (endTime=" + request.getParameter("endTime")
				+ ")");

		// Grab the number of hours offset
		String hoursOffset = request.getParameter("hoursOffset");
		logger.debug(" --> (hoursOffset="
				+ request.getParameter("hoursOffset") + ")");
		...
		logger.debug(" --> tryToGraphBoolean=" + tryToGraphBoolean
				+ "(tryToGraph=" + request.getParameter("tryToGraph") + ")");

		// Grab the user's chosen data delimiter
		String graphDelimiter = request.getParameter("graphDelimiter");
		logger.debug(" --> (graphDelimiter="
				+ request.getParameter("graphDelimiter") + ")");
		...
		logger.debug(" --> htmlFormatBoolean=" + htmlFormatBoolean
				+ "(htmlFormat=" + request.getParameter("htmlFormat") + ")");
		...
		if ((outputDelimiter == null)
				|| (outputDelimiter.compareTo("") == 0)) {
			outputDelimiter = ",";
		}
		logger.debug(" --> outputDelimiter=" + outputDelimiter
				+ "(outputDelimiter="
				+ request.getParameter("outputDelimiter") + ")");
		...
		if ((deviceID != null) && (!deviceID.equals(""))) {
			try {
				...
				deviceIDOK = true;
				logger.debug("deviceID of " + deviceIDLong + " look valid");
			} catch (Exception ex) {
				deviceIDLong = null;
				deviceIDOK = false;
				out.println("The device ID does not appear to be a number");
				logger.error("Could not convert deviceID of " + deviceID
						+ " to a Long");
			}
		} else {
			out.println("A Valid Device ID was not found");
			logger.error("A valid Device ID was not found");
		}
			...
			try {
				...
				while ((inputLine = in.readLine()) != null) {
					...
					if (inputLine.startsWith("embargoed.devices")) {
						...
						String afterEquals = inputLine.substring(inputLine
								.indexOf("=") + 1);

						// Break into array of devices
						String[] devices = afterEquals.split(",");
						if (ArrayUtils
								.indexOf(devices, deviceID.toString()) >= 0) {
							// Now make sure requester is inside MBARI
							if (!request.getRemoteAddr().startsWith(
									"134.89.")) {
								logger.info("Outside MBARI request ("
										+ request.getRemoteAddr()
										+ ") was made for device "
										+ deviceIDLong
										+ " but it is embargoed, request will be denied");
								deviceIDLong = null;
								deviceIDOK = false;
								out.println("Device is on the embargo list "
										+ "and is only available inside MBARI.");
							}
						}
					}
				}
		...
		if (deviceIDOK) {
			...
			if (!timeQueryBy.equals("lastNumPackets")) {
				...
				if (timeQueryBy.compareTo("hoursToEnd") == 0) {
					...
					if (timeQueryBy.compareTo("startToHours") == 0) {
						...
						long endTimeInMillis = startCalendar
								.getTimeInMillis()
								+ (hoursOffsetLong.longValue() * 60 * 60 * 1000);
			...
			/* 			try {
			 ...
			 deviceIDLong, null, null, null, null, recordType,
			 null, null, null, null, null,
			 startTimestampSeconds, endTimestampSeconds, null,
			 null, null, null, numberOfPackets, null, null,
			 null, null, null, null, orderBy, true);
			 } catch (Exception ex) {
			 out.println("<h3>Exception getting data: "
			 + ex.getMessage() + "</h3>");
			 }
			 */
			...
			out.print("<div id=\"contentTitle\">Data Report for Device "
					+ deviceID + " sorted by ");
			...
			if (showDateBoolean) {
				...
				if (htmlFormatBoolean) {
					out.print("<th>SIAM Timestamp<br>converted to time/date ("
							+ chosenTimezone + ")</th>");
				} else {
					if (firstColumnYet) {
						out.print(outputDelimiter);
					}
					out.print("SIAM Timestamp coverted to time/date("
							+ chosenTimezone + ")");
				}
				firstColumnYet = true;
			}
			...
			// Use a SSDSByteArrayAccess EJB service
			...
			try {
				...
				out.println("<h3>Exception in lookup: " + e.getMessage()
						+ "</h3>");
			}
			...
			if (startCalendar != null) {
				startTimestampSeconds = new Long(
						startCalendar.getTimeInMillis() / 1000);
			}
			if (endCalendar != null) {
				endTimestampSeconds = new Long(
						endCalendar.getTimeInMillis() / 1000);
			}

			// Set the parameters on the byte array query
			ssdsByteArrayAccessLocal.setDeviceID(deviceIDLong);
			if (recordType != null)
				ssdsByteArrayAccessLocal.setStartPacketSubType(recordType);
			if (startTimestampSeconds != null)
				ssdsByteArrayAccessLocal
						.setStartTimestampSeconds(startTimestampSeconds);
			if (endTimestampSeconds != null)
				ssdsByteArrayAccessLocal
						.setEndTimestampSeconds(endTimestampSeconds);
			if (numberOfPackets != null)
				ssdsByteArrayAccessLocal
						.setLastNumberOfPackets(numberOfPackets);
			if (orderBy != null)
				ssdsByteArrayAccessLocal.addOrderByParameter(orderBy, true);
			...
			while (ssdsByteArrayAccessLocal.hasMoreElements()) {
				byte[] returnedByteArray = ssdsByteArrayAccessLocal
						.nextElement();
				...
				/* 			if (dataMap != null) {
				 ...
				 logger.info("Data object is of class "
				 + dataObject.getClass());
				 SSDSDevicePacket ssdsDP = (SSDSDevicePacket) dataObject;
				 */
				...
				if (displayTimestampBoolean) {
					if (htmlFormatBoolean) {
						out.print("<td><xmp>" + ssdsDP.systemTime()
								+ "</xmp></td>");
					} else {
						if (firstColumnYet) {
							out.print(outputDelimiter);
				...
				if (showDateBoolean) {
					...
					if (currentCalendar != null) {
						currentCalendar
								.setTimeInMillis(ssdsDP.systemTime());
						if (dateTimeConversionFormat.compareTo("oasis") == 0) {
							double dayOfYear = (double) currentCalendar
									.get(Calendar.DAY_OF_YEAR);
							double hourOfDay = (double) currentCalendar
									.get(Calendar.HOUR_OF_DAY);
							double minute = (double) currentCalendar
									.get(Calendar.MINUTE);
							double second = (double) currentCalendar
									.get(Calendar.SECOND);
							double fracDay = hourOfDay + minute / 60.0
									+ second / 3600.0;
							...
							dateString = df.format(currentCalendar
									.getTime());
						}
						// If html format is on, print out Date with table header tags
						if (htmlFormatBoolean) {
							out.print("<td><xmp>" + dateString
									+ "</xmp></td>");
						} else {
							if (firstColumnYet) {
								out.print(outputDelimiter);
				...
				if (displaySequenceNumberBoolean) {
					if (htmlFormatBoolean) {
						out.print("<td><xmp>" + ssdsDP.sequenceNo()
								+ "</xmp></td>");
					} else {
						if (firstColumnYet) {
							out.print(outputDelimiter);
				...
				if (displayDataBuffer) {
					if (convertToHexBoolean) {
						...
						String godsDeviceParameter = "?deviceID="
								+ deviceID;
						String godsEndTimestampSeconds = "&endTimestampSeconds="
								+ ssdsDP.systemTime() / 1000;
						String godsEndTimestampNanoseconds = "&endTimestampNanoSeconds="
								+ (ssdsDP.systemTime() - ssdsDP
										.systemTime() / 1000) * 1000;
						String godsRest = "&lastNumberOfPackets=1"
								+ "&outputAs=binary"
								+ "&displayPacketHeaderInfo=false"
								+ "&noHTMLHeader=true";
						String imageTagString = "<img src=\"" + godsBase
								+ godsDeviceParameter
								+ godsEndTimestampSeconds
				...
				if (displaySecondDataBufferSize) {
					...
					// Print out the buffer size
					if (htmlFormatBoolean) {
						out.print("<td><xmp>" + dataBufferLength
								+ "</xmp></td>");
					} else {
						if (firstColumnYet) {
							out.print(outputDelimiter);
				...
				if (displaySecondDataBuffer) {
					...
					if (convertSecondToHexBoolean) {
						...
						for (int hexCounter = 0; hexCounter < dataBuffer.length; hexCounter++) {
							hexData.append(Integer
									.toHexString(dataBuffer[hexCounter]));
						}
						if (htmlFormatBoolean) {
			...
			/* 			if (dataMap != null) {
			 ...
			 SSDSGeoLocatedDevicePacket ssdsDP2 = (SSDSGeoLocatedDevicePacket) iterator2
			 .next();
			 ...
			 try {
			 dataPattern = Pattern
			 .compile("^[^0-9&&[^-]]*(-*\\d+\\.*\\d*)([[eE][+-]\\d+]*)\\D*");
			 } catch (Exception ex) {
			 out.println("<b>Could not compile data pattern</b>");
			 }
			 String firstByteArray = new String(
			 ssdsDataPacket.getDataBuffer());
			 String[] columns = firstByteArray
			 ...
			 out.println("<b>Could not extract data from text in column "
			 + columnIndex + "</b>");
			 }
			 }
			 ...
			 XYSeries[] dataSeries = new XYSeries[validColumns
			 .size()];
			 double[] minValues = new double[validColumns
			 .size()];
			 double[] maxValues = new double[validColumns
			 .size()];
			 for (int xyIndex = 0; xyIndex < validColumns
			 .size(); xyIndex++) {
			 ...
			 * Double.MAX_VALUE;
			 ...
			 Iterator iGraphKeySet = graphKeySet
			 .iterator();
			 while (iGraphKeySet.hasNext()) {
			 Long graphKey = (Long) iGraphKeySet
			 .next();
			 // Grab the collection of SSDSDataPackets
			 Collection ssdsDataPackets = (Collection) dataMap
			 .get(graphKey);
			 // Now iterate over those to get SSDSDevicePackets
			 Iterator ssdsDataPacketsIter = ssdsDataPackets
			 .iterator();
			 while (ssdsDataPacketsIter.hasNext()) {
			 // Grab the SSDSDevicePacket
			 SSDSDevicePacket ssdsDevicePacket = (SSDSDevicePacket) ssdsDataPacketsIter
			 .next();
			 if (ssdsDevicePacket
			 .getPacketType() == 1) {
			 ...
			 String[] currentDataColumns = graphData
			 .split(delimiterPattern);
			 // Now for each valid column, grab the key
			 for (int validColumnIndex = 0; validColumnIndex < validColumns
			 .size(); validColumnIndex++) {
			 int column = ((Integer) validColumns
			 .get(validColumnIndex))
			 .intValue();
			 try {
			 Matcher m = dataPattern
			 .matcher(currentDataColumns[column]);
			 boolean b = m.matches();
			 if (b) {
			 boolean createdDouble = true;
			 ...
			 && (exponentString
			 .compareTo("") != 0)) {
			 String exponentValue = exponentString
			 .substring(1);
			 if (exponentValue
			 .charAt(0) == '+') {
			 ...
			 * (Math.pow(
			 ...
			 if (createdDouble) {
			 if (putDouble
			 .doubleValue() > maxValues[validColumnIndex]) {
			 maxValues[validColumnIndex] = putDouble
			 .doubleValue();
			 }
			 if (putDouble
			 .doubleValue() < minValues[validColumnIndex]) {
			 minValues[validColumnIndex] = putDouble
			 .doubleValue();
			 }
			 (dataSeries[validColumnIndex])
			 .add(graphKey
			 .longValue(),
			 putDouble
			 .doubleValue());
			 ...
			 out.println("<h3>Chart was not created because line would be flat with value of "
			 + minValues[xySeriesIndex]
			 + "</h3>");
			 continue;
			 }
			 ...
			 XYPlot xyPlot = new XYPlot(xyColl,
			 xAxis, yAxis, renderer);
			 // Hmmmm
			 Range range = yAxis.getRange();
			 if (range.getLowerBound() == range
			 .getUpperBound()) {
			 yAxis.setRange(
			 range.getLowerBound()
			 - Math.abs(range
			 ...
			 * 0.01);
			 ...
			 Double whackedRange = new Double(
			 "4.0E-320");
			 range = yAxis.getRange();
			 if (range.getUpperBound()
			 - range.getLowerBound() < whackedRange
			 .doubleValue()) {
			 ...
			 + filename
			 + "\" width=1024 height=400 border=0 usemap=\"#"
			 + filename + "\"></img><br>");
			 }
			 }
			 } else {
			 messageToUser
			 .append("The delimiter was not reckognized, so no chart was created");
			 }
			 }
