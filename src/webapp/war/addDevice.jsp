...
<body id="body">
	...
	<%
		// A logger
		Logger logger = Logger.getLogger("moos.ssds.web.addDevice.jsp");
		...
		if (paramsOK) {
			...
			try {
				...
				logger.error("Exception caught trying to get local interfaces to EJBs: "
						+ ex.getMessage());
			}
			...
			try {
				...
				logger.error("Exception caught trying to get device with those characteristics: "
						+ dae.getMessage());
			}
			...
			if ((devices == null) || (devices.size() <= 0)) {
				...
				if ((deviceType != null) && (!deviceType.equals(""))) {
					try {
						dt = (DeviceType) deviceTypeAccessLocal.findById(
								deviceType, false);
					} catch (Exception dae) {
					}
				}
				...
				if (problemWithFormData) {
					...
					try {
						...
						logger.error("Exception caught trying to persist device: "
								+ dae.getMessage());
					}
					...
					out.println("<li>Manufacturer Name = " + d.getMfgName()
							+ "</li>");
