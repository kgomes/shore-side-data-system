 ...
 */
@Stateless
@RemoteBinding(jndiBinding = "moos/ssds/services/data/GeospatialGraphingAccess")
@LocalBinding(jndiBinding = "moos/ssds/services/data/GeospatialGraphingAccessLocal")
public class GeospatialGraphingAccessEJB implements GeospatialGraphingAccess,
		GeospatialGraphingAccessLocal {
	...
	@Resource(mappedName = "moos/ssds/services/data/DeviceDataAccess")
	 ...
	 * 
	 * @see
	 * moos.ssds.services.data.graphing.GeospatialGraphingAccess#getGpsChart
	 * (moos.ssds.metadata.Device, java.util.Date, java.util.Date, boolean,
	 * double, boolean, boolean, double, double, int, int, java.lang.String)
	 */
	@Override
	public String getGpsChart(Device device, Date startDate, Date endDate,
		...
		if (!fileLocationBuffer.toString().endsWith(File.separator)) {
			...
			logger.debug("Appended file separator to file location buffer, now: "
					+ fileLocationBuffer.toString());
		} else {
		...
		if (device.getId() != null) {
			...
			if (!tempDeviceDirectory.exists()) {
				logger.debug("The device directory does not exist, will create it");
				boolean createdDir = tempDeviceDirectory.mkdir();
				logger.debug("Was directory created? " + createdDir);
			} else {
				logger.debug("The directory with the device name already exists");
			}
			...
			logger.debug("After appending the file separator and device ID, file location buffer is "
					+ fileLocationBuffer.toString());
		}
		...
		logger.debug("Converted data collection to an array of LocationAndTime object ("
		...
		GpsCharter gpsCharter = new GpsCharter(gpsLocations, drawWatchCircle,
				watchCircleDiameterInKm, scaleChartToFitData,
				plotAnchorLocation, anchorLocation);
