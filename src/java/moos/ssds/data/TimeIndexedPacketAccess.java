...
public class TimeIndexedPacketAccess implements ITimeIndexedDataAccess {
	...
	/**
	 * This is the constructor that takes in the <code>DataContainer</code> and
	 * a time window to load the data for access. It also accepts a
	 * <code>Collection</code> of <code>String</code>s that are the variable
	 * names of the variables to pull the data for. If the the collection is not
	 * specified, all record variables will be read in.
	 * 
	 * @param dataContainer
	 * @param startDate
	 * @param endDate
	...
	private void initializeData() throws DataException {
		...
		try {
			sqlDSLocal = (SQLDataStreamRawDataAccessLocal) namingContext
		...
		if (sqlDSLocal == null)
			try {
				sqlDS = (SQLDataStreamRawDataAccess) namingContext
						.lookup("moos/ssds/services/data/SQLDataStreamRawDataAccess");
			} catch (NamingException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
		...
		while (getParser().hasNext()) {
			try {
				...
				logger.debug("Failed to parse packet contents for DataContainer : "
				...
				for (Iterator iter = packetMapRecordVariablesToData.keySet()
					...
					if (vList != null) {
						...
						if (o != null) {
							...
							logger.error("object is null, trouble with record variable "
	...
	public Object[] getTime() {
		...
		return MathUtil.orderVector(
				(Long[]) times.toArray(new Long[times.size()]), sortOrder);
	}
	...
	/**
	 * This is the <code>DataContainer</code> that this data access object uses
	 * to read data from
	 */
