 ...
 * <p/>
 * copyright (c) 1999-2007 Roedy Green, Canadian Mind Products may be copied and
 * used freely for any purpose but military.
 * <p/>
 * Encode arbitrary binary into printable ASCII using BASE64 encoding. very
 * loosely based on the Base64 Reader by: Dr. Mark Thornton<br>
 ...
 * <p/>
 * version history vension 1.9, 2007-05-20 -- add icon and pad version 1.8,
 * 2007-03-15 -- tidy. version 1.7 2007-03-15 -- add Example version 1.4 2002
 * February 15 -- correct bugs with uneven line lengths, allow you to configure
 * line separator. now need Base64 object and instance methods. new mailing
 * address.
 * <p/>
 * version 1.3 2000-09-12 -- fix problems with estimating output length in
 * encode
 * <p/>
 * version 1.2 2000-09-09 -- now handles decode as well.
 * <p/>
 * version 1.1 1999-12-04 -- more symmetrical encoding algorithm. more accurate
 * StringBuffer allocation size.
 * <p/>
...
public class Base64 {
	 ...
	 * 
	 * @param s
	 *            is the String of encoded characters to decode and return as a
	 *            byte array (base64-encoded string)
	 * @return decode byte array as plain text
	 */
	public byte[] decode(String s) {
		...
		if (cycle != 0) {
			throw new ArrayIndexOutOfBoundsException("Input to decode not an "
					+ "even multiple of 4 characters; pad with " + spec3);
		}
		j -= dummies;
		if (b.length != j) {
			byte[] b2 = new byte[j];
	...
	public String encode(byte[] b) {
		...
		case 2:
			...
			sb.append(encode(new byte[] { b[len], b[len + 1], 0 }).substring(0,
					3));
			sb.append(spec3);
			break;
		}// end switch;
	...
	/**
	 * Initialize both static and instance table.
	 */
	private void initTables() {
		// initialize valueToChar and charToValue tables
		if (vc == null) {
			// statics are not initialized yet
			vc = new char[64];
			cv = new int[256];
	...
	/**
	 * Main method just to see if it will run
	 * 
	 * @param args
	 *            not used
	 */
