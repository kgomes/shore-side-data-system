<%@ page import="java.io.*,java.util.*,moos.ssds.metadata.*,javax.naming.*,moos.ssds.services.metadata.*,javax.ejb.*" %>
...
if ((name == null) || (name.compareTo("") == 0)) {
  paramsErrorMessage.append("<li>No device name was entered</li>");
  paramsOK = false;
}
...
if (paramsOK) {
    ...
    // The LocalHome interface to the deviceType service
    ...
    try {
        ...
        out.println("NamingException: Could not find the SSDS Services: " + e.getMessage());
    }
    // Check to see if the home interfaces were found
    if ((personAccessLocalHome != null) && (deviceAccessLocalHome != null)) {
        // Grab the person access service and the device access service
        try {
            ...
            out.println("Could not create local interfaces");
        } catch (Exception e) {
            out.println("Could not create local interfaces");
        }
        ...
        try {
            devices = deviceAccessLocal.findByNameAndMfgInfo(name, true, mfgName, true, mfgModel, true, mfgSerialNumber, true, null, null, false);
	} catch (Exception dae) {
	}
	...
	if ((devices == null) || (devices.size() <= 0)) {
            ...
            Person p = null;
            try {
                p = (Person) personAccessLocal.findById(person,false);
            } catch (Exception dae) {
                out.println("<h2>Exception " + dae.getMessage() + "</h2>");
            }
            // Find the device type as well
            DeviceType dt = null;
            if ((deviceType != null) && (!deviceType.equals(""))) {
                try {
                    dt = (DeviceType) deviceTypeAccessLocal.findById(deviceType,false);
                } catch (Exception dae) {
                }
            }
            ...
            if (problemWithFormData) {
                out.println("<h2>There was a problem with the data that you entered, please go back and try again: " + errorMessage + "</h2>");
            } else {
                // Now persist it
                Long deviceIDReturned = null;
                try {
                    deviceIDReturned = deviceAccessLocal.makePersistent(d);
                } catch (Exception dae) {
                }
                ...
                out.println("<ul>");
                out.println("<li><b>Device ID = " + d.getId() + "</b></li>");
                out.println("<li><b>Device UUID = " + d.getUuid() + "</b></li>");
                out.println("<li>Device Name = " + d.getName() + "</li>");
                out.println("<li>Device Description = " + d.getDescription() + "</li>");
                if (d.getDeviceType() != null)
                    out.println("<li>Device Type = " + d.getDeviceType().getName() + "</li>");
                out.println("<li>Manufacturer Name = " + d.getMfgName() + "</li>");
                out.println("<li>Manufacturer Model = " + d.getMfgModel() + "</li>");
                out.println("<li>Manufacturer Serial Number = " + d.getMfgSerialNumber() + "</li>");
            }
        } else {
            out.println("<b>A device with name " + name + ", manufacturer name of " + mfgName + 
                ", manufacturer model of " + mfgModel + ", and manufacturer serial number of " + mfgSerialNumber +
