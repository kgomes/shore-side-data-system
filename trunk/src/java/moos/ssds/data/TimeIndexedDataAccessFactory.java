...
public class TimeIndexedDataAccessFactory {
     ...
     * given time window. If a collection of <code>String</code>s are
     * specified, it will limit the <code>RecordVariable</code>s available to
     * those specified in the collection (otherwise it will use all).
     * 
     * @param dataContainer
     * @param startDate
     * @param endDate
     ...
     *            <code>DataContainer</code> that the data access is to be
     *            setup for
     * @param startDate
     *            is the inclusive <code>Date</code> that specifies the start
     *            of the data that will be available in the data access object
     * @param endDate
    ...
    public static ITimeIndexedDataAccess getTimeIndexedDataAccess(String URL,
        ...
        // then streams)
        DataContainerAccessLocalHome dcAccessLocalHome = null;
        DataContainerAccessLocal dcAccessLocal = null;
        try {
            dcAccessLocalHome = DataContainerAccessUtil.getLocalHome();
            dcAccessLocal = dcAccessLocalHome.create();
        } catch (NamingException e5) {
            logger.debug("NamingException caught trying to create local "
                + "interface to EJB methods for DataContainerAccess: "
                + e5.getMessage());
            logger.debug("This might be OK as it was trying to find local "
                + "interfaces and they may be remote, will try the "
                + "remote shortly");
        } catch (CreateException e5) {
            logger.debug("CreateException caught trying to create local "
                + "interface to EJB methods for DataContainerAccess: "
                + e5.getMessage());
            logger.debug("This might be OK as it was trying to find local "
                + "interfaces and they may be remote, will try the "
        ...
        if (dcAccessLocal != null) {
            ...
            try {
                ...
                logger.error("RemoteException caught trying to get remote "
                    + "interfaces to DataContainerAccess:" + e.getMessage());
            } catch (NamingException e) {
                logger.error("NamingException caught trying to get remote "
                    + "interfaces to DataContainerAccess:" + e.getMessage());
            } catch (CreateException e) {
                logger.error("CreateException caught trying to get remote "
                    + "interfaces to DataContainerAccess:" + e.getMessage());
            }
            ...
            if (dataContainerAccess != null) {
                ...
                try {
                    ...
                    logger
                        .error("RemoteException caught trying to find DataContainer "
                    ...
                    logger
                        .error("MetadataAccessException caught trying to find DataContainer "
                            + "that matches the URL "
                            + URL
                            + ": "
                            + e.getMessage());
