...
/**
 * <p>Class for writing <code>DataPackets</code> to a file. This does not serialize
 * objects. Instead, only the data is written out (as binary). </p>
 * <pre>
 *  Use as:
...
public class PacketOutput {
	 ...
	 * <code>setFile(File file)</code> method before calling <code>writeObject</code>
	 */
	public PacketOutput() {
//		logger.debug("Default constructor called");
	 ...
	 * 
	 * @param file is the <code>File</code> where the 
...
//		logger.debug("Constructor called with input file: " + file);
	...
	/**
	 * Specifies the location to write <code>SSDSDevicePackets</code> to. If the file
	 * already exists new data will be appended to the file. <br><br>
	 *
	 * Calling <code>setFile</code> closes the existing location and opens a new one. So
	 * <code>PacketOutput</code> can be redirected while <code>PacketOutput</code> is running.
	 * 
	 * @param file is the <code>File</code> where the <code>SSDSDevicePackets</code> will
	 * be written to.
	 */
...
//		logger.debug("file.getAbsolutePath() = " + file.getAbsolutePath());
	...
	/**
	 * This method returns the <code>File</code> where this <code>PacketOutput</code>
	 * is serializing <code>SSDSDevicePackets</code> to.
	 * 
	 * @return The <code>File</code> that an instance of PacketOutput is writing to.
	 */
	public File getFile() {
		return file;
	}
	...
	/**
	 * This is the method used to serialize <code>SSDSDevicePacket</code>s to
	 * a file.  It calls private methods that format the output depending on the
	 * VERSION_ID of the <code>SSDSDevicePacket</code> being written.
	 * 
	 * @param packet Is the <code>SSDSDevicePacket</code> to be serialized
	 */
...
//				logger.debug("Serializing version 3");	
	...
	/**
	 * This writes the byte array out in the current version.  There
	 * really is no "version" control here except with what is hard
	 * coded.
	 * @param bytes
	 */
	public synchronized void writeBytesMessage(BytesMessage bytesMessage) throws IOException {
		this.writeBytesMessageVersion3(bytesMessage);
		// Flush the output
		this.flush();
	}
	/**
	 * This is the method to serialize the first version of <code>SSDSDevicePacket</code>
	 * to a file
	 * 
	 * @param packet is the <code>SSDSDevicePacket</code> to serialize
...
//		logger.debug("writeVersion 1 called");
	...
	/**
	 * This is the write method to record the second version of packet to a stream.
	 * This was added so that a second byte array was written to the serialization
	 * mechanism.
	 * 
	 * @param packet
	 * @throws IOException
...
//		logger.debug("writeVersion2 called");
	...
	/**
	 * This is the write method to record the third version of packet to a stream.
	 * This was added so that a second byte array was written to the serialization
	 * mechanism.
	 * 
	 * @param packet
...
//		logger.debug("writeVersion3 called");
		out.writeInt(SSDSDevicePacket.VERSION_ID);
		out.writeLong(packet.sourceID());
		out.writeLong(packet.getPlatformID());
		out.writeInt(packet.getPacketType());
		out.writeLong(packet.getRecordType());
		out.writeLong(packet.getMetadataSequenceNumber());
		out.writeLong(packet.getDataDescriptionVersion());
		out.writeLong(packet.systemTime()/1000);
		out.writeLong((packet.systemTime()%1000)*1000);
		out.writeLong(packet.sequenceNo());
		byte[] buffer = packet.getDataBuffer();
		if (buffer != null) {
			out.writeInt(packet.getDataBuffer().length);
	...
	private void writeBytesMessageVersion3(BytesMessage bytesMessage) throws IOException {
		...
		try {
			...
			// Write out device ID
			long deviceID = bytesMessage.readLong();
			...
			int bufferLen = bytesMessage.readInt();
			...
			int bufferTwoLen = bytesMessage.readInt();
			...
			ByteArrayInputStream byteArrayIS = new ByteArrayInputStream(bufferBytes);
			while (byteArrayIS.available() > 0) {
				hexData.append(Integer.toHexString( (0xFF&byteArrayIS.read()) | 0x100 ).substring(1));
			}
			StringBuffer hexTwoData = new StringBuffer();
			ByteArrayInputStream byteTwoArrayIS = new ByteArrayInputStream(bufferTwoBytes);
			while (byteTwoArrayIS.available() > 0) {
				hexData.append(Integer.toHexString( (0xFF&byteTwoArrayIS.read()) | 0x100 ).substring(1));
			}
			logger.debug("Got bytesMessage and wrote to disk file " + file.getAbsolutePath() + ":" + 
				"deviceID=" + deviceID + "," +
				"parentID=" + parentID + "," +
		    ...
		    logger.error("Something went wrong in the write:" + e.getMessage());
			throw new IOException("Something went wrong with IO work, a JMSException was caught: " + e.getMessage());
		} catch (Exception e) {
		    logger.error("Something went wrong in the write:" + e.getMessage());
			throw new IOException("Something went wrong with , an Exception was caught: " + e.getMessage());
		}
	}
	/**
	 * This is the method that is called when the object is cleaned up and closes
