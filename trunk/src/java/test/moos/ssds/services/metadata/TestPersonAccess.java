...
public class TestPersonAccess extends TestAccessCase {
    ...
    public void testFindBys() {
        ...
        try {
            ...
            persistentOne = (Person) PERSON_ACCESS.findById(equivalentId, false);
        } catch (RemoteException e) {
            assertTrue("RemoteException was thrown: " + e.getMessage(), false);
        } catch (MetadataAccessException e) {
            assertTrue("MetadataException was thrown: " + e.getMessage(), false);
        ...
        assertTrue(
            "Collection of usernames should contain personOne username of "
                + PERSON_ONE.getUsername(), usernames.contains(PERSON_ONE
                .getUsername()));
        assertTrue(
            "Collection of usernames should contain PERSON_TWO username of "
                + PERSON_TWO.getUsername(), usernames.contains(PERSON_TWO
                .getUsername()));
