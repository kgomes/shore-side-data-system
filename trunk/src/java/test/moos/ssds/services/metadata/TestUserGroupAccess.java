...
public class TestUserGroupAccess extends TestAccessCase {
    ...
    public void testFindBys() {
        ...
        try {
            ...
            logger
                .error("RemoteException caught inserting userGroups in find by test: "
                    + e.getMessage());
            assertTrue(
                "RemoteException caught inserting userGroups in find by test: "
                    + e.getMessage(), false);
        } catch (MetadataAccessException e) {
            logger
                .error("MetadataAccessException caught inserting userGroups in find by test: "
                    + e.getMessage());
            assertTrue(
        ...
        try {
            ...
            assertTrue("RemoteException caught trying to findId(UserGroup):"
                + e.getMessage(), false);
        } catch (MetadataAccessException e) {
            assertTrue(
        ...
        try {
            ...
            assertTrue(
                "RemoteException caught trying to findEquivalentPersistentObject(UserGroup):"
                    + e1.getMessage(), false);
        } catch (MetadataAccessException e) {
            assertTrue(
        ...
        assertTrue("findAll should have USER_GROUP_ONE", allUserGroups
            .contains(USER_GROUP_ONE));
        assertTrue("findAll should have USER_GROUP_TWO", allUserGroups
            .contains(USER_GROUP_TWO));
        ...
        try {
            ...
            assertTrue("RemoteException caught trying to findByGroupName():"
                + e.getMessage(), false);
        } catch (MetadataAccessException e) {
            assertTrue(
        ...
        assertNotNull("findByGroupName(JUnit UserGroupOne) should found something",
        ...
        assertTrue("allGroupNames should have two more entries", allGroupNames
            .size() == (countBeforeInserts + 2));
        // Now make sure the ids and names are all in the returned collection
        assertTrue("UserGroup ID one should be there: ", allIds
            .contains(userGroupOneId));
        assertTrue("UserGroup ID two should be there: ", allIds
            .contains(userGroupTwoId));

        assertTrue("UserGroup name one should be there: ", allGroupNames
            .contains("JUnit UserGroupOne"));
        assertTrue("UserGroup name two should be there: ", allGroupNames
            .contains("JUnit UserGroupTwo"));
