...
public class TestPacketOutput extends TestCase {
	...
	public void testWriteToFile() {
		...
		packetOutput.writeBytes(PacketUtility.createSSDSFormatByteArray(
		...
		try {
			...
			assertEquals("ParentIDs should be the same", parentID, lastPacket
					.getParentId());
			assertEquals("PacketType should be the same", packetType,
					lastPacket.getPacketType());
			...
			assertEquals("MetadataSequenceNumber should be the same",
					metadataSequenceNumber, lastPacket
							.getMetadataSequenceNumber());
			assertEquals("DataDescriptionVerion should be the same",
					dataDescriptionVersion, lastPacket
							.getDataDescriptionVersion());
			assertEquals("TimestampSeconds should be the same",
					timestampSeconds, lastPacket.getTimestampSeconds());
			...
			assertTrue("FirstBuffer should be the same", Arrays.equals(
					firstBuffer, lastPacket.getDataBuffer()));
